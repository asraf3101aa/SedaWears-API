...
public class GetShopProductsValidator : PaginatedQueryValidator<GetShopProductsQuery> { }

public class GetShopProductsHandler(
    IApplicationDbContext dbContext,
    IUserService userService,
    ICurrentUser currentUser,
    IOriginContext originContext,
    IOptions<OpeninaryConfig> configOptions) : IRequestHandler<GetShopProductsQuery, PaginatedList<ProductDto>>
{
    public async Task<PaginatedList<ProductDto>> Handle(GetShopProductsQuery request, CancellationToken ct)
    {
        var shopQuery = dbContext.Shops.AsNoTracking().Where(s => s.Id == request.ShopId);

        var query = dbContext.Products
            .AsNoTracking()
            .Where(p => p.Category.ShopId == request.ShopId);
        ...
        if (originContext.OriginRole == UserRole.Customer)
        {
            ...
            query = query.Where(p => p.IsActive && !p.IsDeleted && p.Category.IsActive && !p.Category.IsDeleted);
        }
        else
        {
            ...
            query = query.Where(p => !p.Category.IsDeleted);
        }
